        ...
        public class Channel : IDisposable {
            ...
            internal WhoAmIModel _whoAmI(){
                ...
                if(SendSuccessfully(cmd, Timeout))
                    ...
                    throw new WhoAmIException(cmd.ErrorMessage);
            }
            /// <summary>
            ...
            private void HandleReply (string line) {
                ...
                if (errormatch.Success) {
                    ...
                    try { _responseQueue.Peek () ? .HandleResponse (line); } catch (RegexMatchException) {
                    ...
                    if (!config.AllowedInChannel (client.UniqueId)) {
                        ...
                        if (SpamCount.ContainsKey (client.UniqueId)) {
                            ...
                            if (SpamCount[client.UniqueId] >= 10) {
                                try { BanForSpam (client); } catch (BanException ex) { Console.WriteLine (ex.Message); }
                                return;
                            }
                        } else
                ...
                private void BanlistAdd (string name) {
                    ...
                    } catch (UserNotFoundException) { _sendTextMessage ("Could not find user " + name); } catch (UserNotInChannelException) { } catch (ArgumentException) { _sendTextMessage (name + " is already on the banlist."); }
                }
                            ...
                            if (model.Words[0] == "!help")
                                ...
                                if (model.Words[0] == "!kick") {
                                    ...
                                    if (model.Words.Length > 1) {
                                        ...
                                        } catch (UserNotInChannelException ex) { _sendTextMessage (ex.ClientName + " is not in this channel."); } catch (UserNotFoundException ex) { _sendTextMessage ("Could not find user " + ex.ClientName); }
                                    }
                                }
                            }
                        }
                ...
                private void SetChannelName (string name) {
                    if(!SendSuccessfully(new SetChannelNameCommand(name, ThisChannel.ChannelId),Timeout))
                        throw new SetChannelNameException();
                }
                ...
                private void WhitelistAdd (string name) {
                    ...
                    } catch (UserNotFoundException) { _sendTextMessage ("Could not find user " + name); } catch (ArgumentException) { _sendTextMessage (name + " is already on the whitelist."); }
                }
